...
class TestUi(odoo.tests.HttpCase):
    ...
    def test_manifest_valid_json(self):
        ...
        manifest_data = self.url_open("/damsu_web_pwa/manifest.webmanifest")
    ...
    def test_manifest_correct_paramenters(self):
        ...
        self.assertEquals(
            ...
            "/damsu_web_pwa/static/img/icons/icon-128x128.png",
        )
    ...
    def test_manifest_logo_upload(self):
        with open(
            "%s/static/img/icons/odoo_logo.svg" % get_resource_path("damsu_web_pwa"), "rb"
        ) as fi:
        ...
        self.assertEquals(manifest_content["icons"][0]["src"], "/damsu_web_pwa/icon.svg")
        ...
        icon_data = self.url_open("/damsu_web_pwa/icon.svg")
    ...
    def test_png_logo_upload(self):
        with open(
            "%s/static/img/icons/icon-512x512.png" % get_resource_path("damsu_web_pwa"),
            "rb",
        ) as fi:
            icon_to_send = base64.b64encode(fi.read())
        ...
        expected_vals = {
            "src": "/damsu_web_pwa/icon512x512.png",
            "sizes": "512x512",
            "type": "image/png",
        }
    ...
    def test_manifest_logo_upload_small(self):
        icon_to_send = None
        with open(
            "%s/static/img/icons/icon-128x128.png" % get_resource_path("damsu_web_pwa"),
            "rb",
        ) as fi:
            icon_to_send = base64.b64encode(fi.read())
        # Set PWA icon in settings
